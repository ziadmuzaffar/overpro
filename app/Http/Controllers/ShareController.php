...
{
    private string $root = 'pages.shares.';
    ...
    {
        $shares = Share::where('name', $request->name)->where('university_id', $request->university_id)->where('section_id', $request->section_id)->where('level_id', $request->level_id)->where('group_id', $request->group_id);
        if ($shares->count() == 0) {
    ...
    public function update(Request $request, Share $share)
    {
        foreach ($request->degree as $id => $degree) {
            Share::where('id', $id)->update(['degree' => $degree]);
        }
        return redirect()->route('shares.index');
    }
